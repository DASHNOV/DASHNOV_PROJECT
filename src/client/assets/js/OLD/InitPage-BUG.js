...
async function initialiserPage() {
    ...
    if (!token) {
        ...
        window.location.href = 'http://192.168.200.228:8085/login'; // Rediriger vers la page de login si l'utilisateur n'est pas connecté
        return;
    }
...
function showTab(tabId) {
    ...
    if (tabElement.length > 0) {
        console.log("Affichage de l'onglet : " + tabId); // Vérification
        tabElement.css('display', 'block');
    } else {
        console.warn("L'onglet avec l'ID : " + tabId + " n'a pas été trouvé.");
    }

    if (tabId === 'onglet-generateur-de-rapport') {
        chargerRapports(); // Charger les rapports
    }
}
...
async function updateConnectionStatus() {
    ...
    try {
        const response = await fetch('http://192.168.200.228:8085/verifierConnexion', {
            method: 'GET',
